...
assert.match(background, /settingsVersion: 5/);
...
assert.match(background, /订单仍在官网进行中/);
assert.match(background, /已保留订单，不自动续跑/);
...
assert.match(background, /XINHUO_CONFIRM_AND_WAIT_VERIFICATION/);
assert.match(background, /X 任务组件未找到或未点击提交控件/);
assert.match(background, /async function releaseXinhuoOrderIfNoLongerClaimed/);
assert.match(background, /async function inspectInterruptedXinhuoOrder/);
assert.match(background, /inspection\?\.claimed !== false/);
assert.match(background, /已清理旧订单并返回广场继续扫描/);
assert.match(background, /service worker 重启后确认旧订单已不在进行中/);
